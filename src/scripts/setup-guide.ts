    ...
    .forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          if (label)
            label.textContent = selected ? "Selected" : "Copy unavailable";
          if (status) {
...
const bindKeyChecks = (setup: HTMLElement) => {
  setup
    .querySelectorAll<HTMLFormElement>("[data-key-check-form]")
    .forEach((form) => {
      form.addEventListener("submit", async (event) => {
        event.preventDefault();
        const keyCheck = form.closest("[data-key-check]") as HTMLElement | null;
        const input = form.querySelector<HTMLInputElement>(
          'input[name="hevy-api-key"]',
        );
        const button = form.querySelector<HTMLButtonElement>(
          "[data-key-check-button]",
        );
        const status = keyCheck?.querySelector<HTMLElement>(
...
export const initializeSetupGuide = () => {
  ...
  document
    .querySelectorAll<HTMLAnchorElement>("[data-setup-mode]")
    .forEach((link) => {
  ...
  window.addEventListener("pagehide", () => {
    setup
      .querySelectorAll<HTMLInputElement>('input[name="hevy-api-key"]')
      .forEach((input) => {
        input.value = "";
      });
  });
